...
TPP is a tiny (single source file) C Preprocessor, implementing a lot of preprocessor extensions.

### Usage

TPP is designed to be used as 1 of 2 things:

- A back-end for writing C and C-like compilers with all of the work needed for dealing with Macros, Directives, Warnings, and preprocessor-related Pragmas already done.  
  For this purpose, TPP is meant to be included statically as part of another project's source tree.

- A standalone Preprocessor (similar to GNU's `cpp` utility).  
  For this purpose, you may build `src/frontend.c` to produce the commandline utility


### Features

- TPP operates without any intermediate buffers, in that it is capable of preprocessing/tokenizing text from stream-like input sources:

  ```sh
  cat file.c | tpp
  ```

  Can be used to preprocess text, without any need for any intermediate buffers (beyond what is needed for a single token) before TPP will start producing output.
...
All extensions can be hard enabled/disabled when building tpp, or soft-disabled via `#pragma extension("-fname")` directives.
...
- Support for GCC-style `#warning My Message Here` directives
- Support for GCC-style `#include_next <header.h>` directives
- Support for OBJc-style `#import <header.h>` directives
- Support for `#ident` and `#sccs` directives (true origin unknown; accepted by some ancient compilers for placing comments into `.o`-files)
- Support for magic macros `__FILE__`, `__LINE__`, `__TIME__` and `__DATE__`
- Support for magic macro `__BASE_FILE__`
- Support for magic macros `__INCLUDE_LEVEL__` and `__INCLUDE_DEPTH__`
- Support for magic macro `__COUNTER__`
- Support for magic macro `__TIMESTAMP__`
- Support for magic macro `__COLUMN__` (tpp-specific; like `__LINE__`, but the current column-position)
- STDC-compliant preprocessor `#if`-expressions
- All STDC-required preprocessor directives (`#define`, `#undef`, `#if`, `#ifdef`, `#ifndef`, `#elif`, `#else`, `#endif`, `#include`)
- Support for clang-style feature test macros:
	- `__has_include()`
	- `__has_include_next()`
...
- Recognize `$` as a character which may appear in an identifier
...
- Self-recursive macros (tpp-specific extension):
	- STDC specifies that macros that try to expand to themselves should not be expanded when this happens
	- TPP has an optional extension to relax this rule for function-like macros defined while this extension is active
	- Function-like macros defined while this extension is active are allowed to expand to themselves, so-long as arguments passed during expansion differ from all other invocations of the same macro along the macro-expansion call-stack
...
- Allow for `#@` as a replacement for `#` to convert macro arguments into character literals, rather than string literals:
	- ```c
	  #define STR(x) #x
	  #define CHR(x) #@x
	  STR(f) // "f"
...
- Presive date/time macros (tpp-specific extension):
...
- Generate unique IDs for a given keyword/name (that remain unchanged when re-compiling the same file without altering its contents) (tpp-specific extension)
	- For example, `__TPP_UNIQUE(foo)` expands to a unique decimal integer that will remain the same for `foo` within the entire input (though not necessarily within other files that may also be preprocessed)
...
- More extensions exist, but I feel are not important enough to document here
